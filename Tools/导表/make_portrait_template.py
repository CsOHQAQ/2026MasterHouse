...
这张表只有三列，刻意保持朴素：
    ...
    立绘ID      资源路径                    备注
    fox_平静    OutGameUI/Guests/fox        狐族默认脸
...
资源路径写 **Resources 相对路径、不带扩展名**（如 OutGameUI/Guests/fox）——
...
GUESTS_DIR = os.path.join(ROOT, "Assets", "Resources", "OutGameUI", "Guests")
...
# 四个种族的中文名，纯粹为了让起始内容的备注好读；扫到别的素材就留空
RACE_NAMES = {"fox": "狐族", "crow": "鸦族", "hedgehog": "猬族", "rabbit": "兔族"}


def scan_portraits():
    """扫 Resources/OutGameUI/Guests 下的立绘素材，生成 (立绘ID, 资源路径, 备注)。"""
    ...
    for name in sorted(os.listdir(GUESTS_DIR)):
        ...
        rows.append(("%s_平静" % stem, "OutGameUI/Guests/%s" % stem, note))
    return rows
...
def main():
    ...
    rows = scan_portraits()
    if not rows:
        print("[WARN] no portrait images found under %s" % GUESTS_DIR)
