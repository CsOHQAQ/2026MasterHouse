...
### 2026-08-06 · 起居室背景替换为新版空房子剖面图

- 状态：待验证
- 目标：起居室背景换成美术新图（`Assets/Scenes/Snipaste_2026-08-06_14-00-17.png`，空房子剖面、透明边缘、入口为右侧开放门）；开场揭幕、对话背景、家具烘焙底图同步替换；访客入口/排队/游走坐标适配新布局
- 范围：新图补成 16:9 存为 `Assets/Resources/OutGameUI/house-hub-v3.png`（2062×1160）；PIL 重生成家具三层底图 `room-living-clean/blur/depthblur`（覆盖原文件保 GUID，1672×941 与旧图一致，家具网格坐标无需重标）；`OutGameUIData.Rooms[0].art`、开场 HomeReveal、对话 VisitorScene 两处回退改指 v3；`OutGameVisitorStage` 门口(0.752,0.262)/入口/3 排队点/8 游走点按新图重标（已用坐标叠图预览校验全部落在地板上）
- 不做：厨房仍用旧图 house-hub-v2；家具切片（旧图抠出的沙发茶几等）与房间占位格暂不重制，风格与新背景不一致属已知占位状态
- 验证计划：Play → Hub 背景为新空房图（含家具烘焙合成）；访客从右侧开放门进出、邻居在门前排队；开场揭幕/对话背景为新图；观景模式平移缩放正常
- 离线编译返回 0；透明边缘区域显示画布深色底（剖面悬浮效果），如需纯色底可后续加背景板
