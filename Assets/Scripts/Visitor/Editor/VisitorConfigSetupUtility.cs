...
namespace MasterHouse.EditorTools
{
    ...
    /// 生成物：标签森林（Assets/GameData/Tags，局内局外共用）、示例物资挂 tag、
    ...
    public static class VisitorConfigSetupUtility
    {
        ...
        public static void CreateIfMissing()
        {
            ...
            var fox = Race(created, "fox", "狐族", "OutGameUI/Guests/fox", "OutGameUI/Visitors/orange_cat",
                waitTalk: 3000, waitDeliver: 6000, wanderMax: 3600, stayPercent: 30, needMin: 1, needMax: 2,
                Need(tagWood, 5, true), Need(tagNatural, 3, false), Need(tagCrafted, 2, false));
            var crow = Race(created, "crow", "鸦族", "OutGameUI/Guests/crow", "OutGameUI/Visitors/rottweiler",
                waitTalk: 1800, waitDeliver: 3600, wanderMax: 2400, stayPercent: 10, needMin: 2, needMax: 3,
                Need(tagMaterial, 4, true), Need(tagCrafted, 4, false), Need(tagEnergy, 2, false));
            var rabbit = Race(created, "rabbit", "兔族", "OutGameUI/Guests/rabbit", "OutGameUI/Visitors/xueqiu",
                waitTalk: 4200, waitDeliver: 7200, wanderMax: 6000, stayPercent: 60, needMin: 1, needMax: 1,
                Need(tagNatural, 5, true), Need(tagWood, 3, false));
            var hedgehog = Race(created, "hedgehog", "猬族", "OutGameUI/Guests/hedgehog", "OutGameUI/Visitors/wangcai",
                waitTalk: 2400, waitDeliver: 4800, wanderMax: 3000, stayPercent: 20, needMin: 1, needMax: 2,
                Need(tagEnergy, 4, true), Need(tagCrafted, 3, false));

            // ── 日程表（§4.4：零随机零上限，谁在第几天几点出现由策划配死；加内容请追加表尾）──
        ...
        private static NeedTagWeight Need(TagDef tag, int weight, bool required) =>
            new NeedTagWeight { tag = tag, weight = weight, required = required };
        ...
        private static VisitorRaceDef Race(List<string> created, string id, string displayName,
            ...
            int stayPercent, int needMin, int needMax, params NeedTagWeight[] needs)
        {
            ...
            race.needCountMin = needMin;
            race.needCountMax = needMax;
